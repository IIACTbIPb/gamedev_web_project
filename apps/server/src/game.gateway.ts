...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private applyDamage(targetId: string, damage: number, attackerId?: string) {
    ...
    // Хардкодный Math.min убран! Сервер теперь доверяет параметру damage, 
    // так как он берется напрямую из нашего CLASS_BALANCE
    target.hp -= damage;
  ...
  handleJoinGame(client: TypedSocket, data: JoinGamePayload) {
    ...
    // Берем базовое ХП прямо из конфига баланса. 
    // Если по какой-то причине класса нет, делаем фолбэк на 100
    const classStats = CLASS_BALANCE[data.classType];
    const maxHp = classStats ? classStats.baseHp : 100;
